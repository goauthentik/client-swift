...
[**stagesAuthenticatorMobileEnrollmentCallbackRetrieve**](StagesAPI.md#stagesauthenticatormobileenrollmentcallbackretrieve) | **GET** /stages/authenticator/mobile/{stage_uuid}/enrollment_callback/ | 
...
# **stagesAuthenticatorMobileEnrollmentCallbackRetrieve**
```swift
    open class func stagesAuthenticatorMobileEnrollmentCallbackRetrieve(stageUuid: UUID, completion: @escaping (_ data: AuthenticatorMobileStage?, _ error: Error?) -> Void)
```
...
StagesAPI.stagesAuthenticatorMobileEnrollmentCallbackRetrieve(stageUuid: stageUuid) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
